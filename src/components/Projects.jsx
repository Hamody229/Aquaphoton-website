...
import { Settings, Cpu, Radio, Zap, Anchor, Navigation } from 'lucide-react'; 
      ...
      <div className="max-w-7xl mx-auto relative z-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 lg:gap-12 mb-24">
          {projectList.map((project, index) => (
            <motion.div 
              ...
              viewport={{ once: true }}
              className="group bg-white/5 backdrop-blur-lg border border-white/10 p-6 md:p-8 rounded-[2.5rem] hover:border-blue-500/50 transition-all duration-500 hover:-translate-y-2 flex flex-col h-full"
            >
              {/* Image Area */}
              <div className="h-64 rounded-[2rem] mb-8 overflow-hidden relative border border-white/5">
                <img 
                  src={project.image} 
                  alt={project.title} 
                  className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-700"
                />
                <div className="absolute top-4 right-4 bg-black/60 backdrop-blur-md px-4 py-1 rounded-full text-[10px] font-mono uppercase tracking-widest text-blue-400 border border-white/10">
                  {project.category}
                </div>
              </div>
              ...
              <div className="flex-grow">
                <h3 className="text-3xl font-bold text-white uppercase italic leading-none mb-1">
                  {project.title}
                </h3>
                ...
                <ul className="space-y-4">
                  {project.features.map((feature, i) => (
                    <li key={i} className="flex items-start gap-3 text-slate-300 text-sm md:text-base font-light leading-relaxed">
        ...
        >
          <p className="text-xl md:text-3xl font-bold text-white/90 italic leading-relaxed">
            "Each project is <span className="text-blue-500">built</span>, tested, <span className="text-red-500">failed</span>, <span className="text-green-500">improved</span>, and tested again."
          </p>
